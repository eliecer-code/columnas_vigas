...
public class ElementGenerationService : IElementGenerationService
{
    private void LogStep(string step)
    {
        try
        {
            string path = System.IO.Path.Combine(System.Environment.GetFolderPath(System.Environment.SpecialFolder.ApplicationData), "CQIng_Diagnostic.log");
            System.IO.File.AppendAllText(path, $"[{DateTime.Now:yyyy-MM-dd HH:mm:ss.fff}] {step}\n");
        }
        catch { }
    }
        ...
        long framingTypeId)
    {
        ...
        try
        {
            ...
            LogStep(currentStep);
            using (TransactionGroup transGroup = new TransactionGroup(doc, "Generar Columnetas y Viguetas"))
            {
                transGroup.Start();
                ...
                using (Transaction t = new Transaction(doc, "Generar Elementos Estructurales"))
                {
                    ...
                    Dictionary<ElementId, Curve> originalCurves = new Dictionary<ElementId, Curve>();
                    ...
                    bool IsColumnNearby(XYZ pt, double minZ, double maxZ)
                    {
                        ...
                        foreach(var col in existingCols)
                        {
                            ...
                            if (bb != null)
                            {
                                bool overXY = pt.X >= bb.Min.X - tolXY && pt.X <= bb.Max.X + tolXY &&
                                              pt.Y >= bb.Min.Y - tolXY && pt.Y <= bb.Max.Y + tolXY;
                                bool overZ = !(maxZ < bb.Min.Z - tolXY || minZ > bb.Max.Z + tolXY);
                                if (overXY && overZ) return true;
                            }
                        }
                        return false;
                    }
                    ...
                    foreach (var wModel in selectedWalls)
                    {
                        ...
                        if (wall == null) throw new InvalidOperationException($"El muro con Id {wModel.Id} no existe o no es de tipo Wall.");
                        
                        currentStep = $"Lectura de la LocationCurve del muro {wModel.Id}";
                        LogStep(currentStep);
                        LocationCurve locCurve = wall.Location as LocationCurve;
                        if (locCurve == null) throw new InvalidOperationException($"El muro {wModel.Id} no tiene LocationCurve.");
                        
                        originalCurves[wall.Id] = locCurve.Curve;

                        currentStep = $"Dimensiones del muro {wModel.Id}";
                        LogStep(currentStep);
                        var (zMinW, zMaxW, bLevelW) = WallGeometryHelper.GetWallElevationInfo(doc, wall);
                        if (bLevelW == null) throw new InvalidOperationException($"El muro {wModel.Id} no tiene un nivel base válido.");
                        if (wall.Width <= 0) throw new InvalidOperationException($"El muro {wModel.Id} tiene un espesor inválido.");
                        if (zMaxW <= zMinW) throw new InvalidOperationException($"El muro {wModel.Id} tiene una altura inválida.");
                        if (locCurve.Curve.Length <= 0) throw new InvalidOperationException($"El muro {wModel.Id} tiene una longitud inválida.");

                        processedWalls.Add(wall);
                    }

                    List<CornerNode> nodes = CornerNodeSolver.BuildTopologicalNodes(doc, processedWalls);
                    ...
                    if (baseFramingType != null && processedWalls.Any())
                    {
                        ...
                        var (zwMin, zwMax, bLevel) = WallGeometryHelper.GetWallElevationInfo(doc, sampleWall);
                        double wThickness = sampleWall.Width;
                        FamilySymbol tempFramingType = FamilySymbolHelper.GetOrDuplicateSymbolWithWidth(doc, baseFramingType, wThickness);
                        ...
                        using (SubTransaction subT = new SubTransaction(doc))
                        {
                            ...
                            FamilyInstance tempBeam = doc.Create.NewFamilyInstance(tempLine, tempFramingType, bLevel, Autodesk.Revit.DB.Structure.StructuralType.Beam);
                    ...
                    foreach (var node in nodes)
                    {
                        ...
                        FamilySymbol columnType = FamilySymbolHelper.GetOrDuplicateSymbolWithWidth(doc, baseColumnType, wallThickness);
                        ...
                        XYZ finalColRight = RealGeometryHelper.GetExtremePoint(col, doc, -normalDir);
                        
                        if (finalColFront != null && finalColBack != null && finalColLeft != null && finalColRight != null)
                        {
                            XYZ currentCenter = (finalColFront + finalColBack + finalColLeft + finalColRight) / 4.0;
                            double distCenter = currentCenter.DistanceTo(adjustedTargetCenter);
                            
                            if (distCenter > 0.01)
                            {
                                string logMsg = $"[VALIDACION FALLIDA NODO {colIndex}]\n" +
                            ...
                            {
                                LogStep($"[VALIDACION EXITOSA NODO {colIndex}] Geometría alineada perfectamente. Tolerancia < 0.01");
                            }
                        }
                    ...
                    foreach (var wall in processedWalls)
                    {
                        ...
                        for (int i = 0; i < N; i++)
                        {
                            ...
                            XYZ transOffset = WallConfinementCalculator.CalculateTransversalAlignmentOffset(wall, columnType, dir);
                    ...
                    if (baseFramingType != null)
                    {
                        ...
                        foreach (var wall in processedWalls)
                        {
                            ...
                            FamilySymbol framingType = FamilySymbolHelper.GetOrDuplicateSymbolWithWidth(doc, baseFramingType, wallThickness);
                            ...
                            FamilyInstance botFraming = doc.Create.NewFamilyInstance(botBeamLine, framingType, baseLevel, Autodesk.Revit.DB.Structure.StructuralType.Beam);
                    ...
                    foreach(var framing in createdFramings)
                    {
                        var intWalls = new FilteredElementCollector(doc)
                            ...
                            .ToList();
                        foreach(var wall in intWalls) JoinAndCut(doc, cutter: framing, cuttee: wall);
                        ...
                        var intCols = new FilteredElementCollector(doc)
                            ...
                            .WherePasses(new ElementIntersectsElementFilter(framing))
                            .ToList();
                        foreach(var c in intCols) JoinAndCut(doc, cutter: c, cuttee: framing);
                    }
        ...
        catch (Exception ex)
        {
            ...
            td.MainContent = $"Paso actual: {currentStep}\n\n" +
                             $"Mensaje: {ex.Message}\n\n" +
    ...
    {
        if (cutter == null || cuttee == null || cutter.Id == cuttee.Id) return;
